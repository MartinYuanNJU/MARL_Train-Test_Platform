...
from agent.common import DDQNArgs
from . import ReplayBuffer, QNet


class DDQNAgent(Agent):

    def __init__(self, args: DDQNArgs):
        self._args = args
    ...
    def set_model(self, parameter_share=False, load_paths=None):
        if not parameter_share:
            ...
            if load_paths is None or not os.path.exists(load_paths[0]):
                if DDQNAgent.networks is None:
                    # [q net, target q net]
                    DDQNAgent.networks = []
                    DDQNAgent.networks.append(QNet(self._args.state_dim, self._action_count))
                    DDQNAgent.networks.append(copy.deepcopy(DDQNAgent.networks[0]).eval())
                self._qnet = DDQNAgent.networks[0]
                self._target_qnet = DDQNAgent.networks[1]
            else:
                if DDQNAgent.networks is None:
                    # [q net, target q net]
                    self._load_model(load_paths)
                    DDQNAgent.networks = []
                    DDQNAgent.networks.append(self._qnet)
                    DDQNAgent.networks.append(self._target_qnet)
                self._qnet = DDQNAgent.networks[0]
                self._target_qnet = DDQNAgent.networks[1]
