...
from agent.common import DuelingDQNArgs
from . import ReplayBuffer, DuelingQNet


class DuelingDQNAgent(Agent):

    def __init__(self, args: DuelingDQNArgs):
        self._args = args
    ...
    def set_model(self, parameter_share=False, load_paths=None):
        if not parameter_share:
            if load_paths is None or not os.path.exists(load_paths[0]):
                self._qnet = DuelingQNet(self._args.state_dim, self._action_count)
                self._target_qnet = copy.deepcopy(self._qnet).eval()
            else:
                self._load_model(load_paths)
        else:
            if load_paths is None or not os.path.exists(load_paths[0]):
                if DuelingDQNAgent.networks is None:
                    # [q net, target q net]
                    DuelingDQNAgent.networks = []
                    DuelingDQNAgent.networks.append(DuelingQNet(self._args.state_dim, self._action_count))
                    DuelingDQNAgent.networks.append(copy.deepcopy(DuelingDQNAgent.networks[0]).eval())
                self._qnet = DuelingDQNAgent.networks[0]
                self._target_qnet = DuelingDQNAgent.networks[1]
            else:
                if DuelingDQNAgent.networks is None:
                    # [q net, target q net]
                    self._load_model(load_paths)
                    DuelingDQNAgent.networks = []
                    DuelingDQNAgent.networks.append(self._qnet)
                    DuelingDQNAgent.networks.append(self._target_qnet)
                self._qnet = DuelingDQNAgent.networks[0]
                self._target_qnet = DuelingDQNAgent.networks[1]
